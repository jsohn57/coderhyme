...
// get each list
var fs = require('fs');
var replyList = fs.list("/projects/GiantEagle/webcrawler/nate/wget_out/spo");
var stream, i, il;
stream = fs.open("/projects/GiantEagle/webcrawler/dat/nate/spo/article_url.in", 'r');
for(i = 0, il = NUM_ARTICLES; i < il; i++){
	articleURLList.push(stream.readLine());
}
stream.close();
stream = fs.open("/projects/GiantEagle/webcrawler/dat/nate/spo/title.in", 'r');
for(i = 0, il = NUM_ARTICLES; i < il; i++){
...
try{
	...
	for(k = 0, kl = replyList.length; k < kl; k++){
		if(replyList[k].indexOf('html') >= 0){	
			visited++;
			if(visited == 1){
				nCasperSports.start("/projects/GiantEagle/webcrawler/nate/wget_out/spo/"+replyList[k]);
			}
			else{
				nCasperSports.thenOpen("/projects/GiantEagle/webcrawler/nate/wget_out/spo/"+replyList[k]);
			}
	...
	nCasperSports.run(function() {
		...
		if(fs.exists("/projects/GiantEagle/webcrawler/dat/crwl/nate_spo.crwl")){
			fs.remove("/projects/GiantEagle/webcrawler/dat/crwl/nate_spo.crwl");
		}
		
		fs.write("/projects/GiantEagle/webcrawler/dat/crwl/nate_spo.crwl", JSON.stringify({
			channel : 'nate_spo',
			at      : new Date(),
			data    : completeList
			}), 'w');

		this.echo("Done writing /projects/GiantEagle/webcrawler/dat/crwl/nate_spo.crwl");
		this.exit(0);
	});
}
