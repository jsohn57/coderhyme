...
var fs = require('fs');
var replyList = fs.list("/projects/GiantEagle/webcrawler/nate/wget_out/m_pol");
var stream, i, il;
...
try{
	...
	nCasperSports.each(replyList, function(self, reply){
		if(reply.indexOf('html') >= 0){	
			visited++;
			if(visited == 1){
				this.start("/projects/GiantEagle/webcrawler/nate/wget_out/m_pol/"+reply);
			}
			else{
				this.thenOpen("/projects/GiantEagle/webcrawler/nate/wget_out/m_pol/"+reply);
			}
			this.then(function(){
				var meta_ = {};
				meta_['article_url'] = "";
	...
	nCasperSports.run(function() {
		...
		if(fs.exists("/projects/GiantEagle/webcrawler/tmp_dat/crwl/m_nate_pol.crwl")){
			fs.remove("/projects/GiantEagle/webcrawler/tmp_dat/crwl/m_nate_pol.crwl");
		}	

		fs.write("/projects/GiantEagle/webcrawler/tmp_dat/crwl/m_nate_pol.crwl", JSON.stringify({
			channel : 'nate_pol',
			at      : new Date(),
			data    : completeList
			}), 'w');

		this.echo("Done writing /projects/GiantEagle/webcrawler/tmp_dat/crwl/m_nate_pol.crwl");
		this.exit(0);
	});
}
