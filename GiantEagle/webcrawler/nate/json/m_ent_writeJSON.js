...
var fs = require('fs');
var stream, i, il;
stream = fs.open("/home/jsohn/coderhyme/coderhyme/GiantEagle/webcrawler/dat/nate/m_ent/article_url.in", 'r');
for(i = 0, il = NUM_ARTICLES; i < il; i++){
	articleURLList.push(stream.readLine());
}
stream.close();
stream = fs.open("/home/jsohn/coderhyme/coderhyme/GiantEagle/webcrawler/dat/nate/m_ent/title.in", 'r');
for(i = 0, il = NUM_ARTICLES; i < il; i++){
...
try{
	...
	nCasperSports.run(function() {
		...
		if(fs.exists("/home/jsohn/coderhyme/coderhyme/GiantEagle/webcrawler/tmp_dat/crwl/m_nate_ent.crwl")){
			fs.remove("/home/jsohn/coderhyme/coderhyme/GiantEagle/webcrawler/tmp_dat/crwl/m_nate_ent.crwl");
		}	

		fs.write("/home/jsohn/coderhyme/coderhyme/GiantEagle/webcrawler/tmp_dat/crwl/m_nate_ent.crwl", JSON.stringify({
			channel : 'nate_ent',
			at      : new Date(),
			data    : completeList
			}), 'w');

		this.echo("Done writing /home/jsohn/coderhyme/coderhyme/GiantEagle/webcrawler/tmp_dat/crwl/m_nate_ent.crwl");
		this.exit(0);
	});
}
